...
## Overview
The Gold Layer is the business-level data representation, structured to support analytical and reporting use cases. It consists of **dimension views** and **fact views**, implemented in MySQL as SQL views on top of the Silver layer, for specific business metrics.
...
## Notes on MySQL implementation
- All `NVARCHAR` types from the original design are implemented as `VARCHAR` in MySQL, since MySQL does not have a separate `NVARCHAR` type (MySQL's `VARCHAR` supports Unicode natively via the table/column character set).
- Gold layer objects are implemented as **views** (`CREATE VIEW`), not physical tables — they are built directly on top of the Silver layer using `LEFT JOIN`s and surrogate keys generated with `ROW_NUMBER() OVER (...)`, which is fully supported in MySQL 8.0+.
